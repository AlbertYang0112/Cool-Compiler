...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    %type <expressions> parallel_expr_list
    %type <expressions> vertical_expr_list
    %type <expression> nest_let_expr
    ...
    */
    ...
    | OBJECTID ':' TYPEID ',' nest_let_expr { $$ = let($1, $3, no_expr(), $5); }
    | OBJECTID ':' TYPEID ASSIGN expr ',' nest_let_expr { $$ = let($1, $3, $5, $7); }
    ;
    ...
    parallel_expr_list
    : /* Empty Expr List */ { $$ = nil_Expressions(); }
    | expr { $$ = single_Expressions($1); }
    | parallel_expr_list ',' expr { $$ = append_Expressions($1, single_Expressions($3)); }
    ;

    vertical_expr_list
    : /* Empty Expr List */ { $$ = nil_Expressions(); }
    | expr ';' {$$ = single_Expressions($1); }
    | vertical_expr_list expr ';' { $$ = append_Expressions($1, single_Expressions($2)); }
    ;

    expr
    : OBJECTID ASSIGN expr { $$ = assign($1, $3); }
    | expr '@' TYPEID '.' OBJECTID '(' parallel_expr_list ')'
    { $$ = static_dispatch($1, $3, $5, $7); }
    | expr '.' OBJECTID '(' parallel_expr_list ')'
    { $$ = dispatch($1, $3, $5); }
    | OBJECTID '(' parallel_expr_list ')'
